...
sf::Texture loadTexture(std::string path)
{
    ...
    {
        std::cout << "Foi com Deus" << std::endl;
        exit(1);
    }
    return texture;
}
...
int main()
{
	sf::Texture fairy_tex { loadTexture("fairy.png") }; 
    sf::Texture cat_tex { loadTexture("cat.png") };
    sf::Texture forest_tex {loadTexture("forest.jpg") };

    const int STEP {100};

	Entity fairy(1, 1, STEP, fairy_tex);
    Entity cat(3, 3, STEP, cat_tex);
    Board board(7, 5, STEP, forest_tex);

    sf::RenderWindow window(sf::VideoMode(board.nc * STEP, board.nl * STEP), "<3 fairy game <3");
	...
	while (window.isOpen())
	{
		...
		while (window.pollEvent(event))
		{
            ...
            {
                moveEntity(event.key.code, fairy, {sf::Keyboard::Left, sf::Keyboard::Up, sf::Keyboard::Right, sf::Keyboard::Down});
                moveEntity(event.key.code, cat, {sf::Keyboard::A, sf::Keyboard::W, sf::Keyboard::D, sf::Keyboard::S});
            }
		}
		window.clear();
        board.draw(window);
